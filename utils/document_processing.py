...
def parse_revision_from_filename(filename: str) -> tuple[str, str | None]:
  ...
  # Extract TI number as base document name (e.g., "TI99", "TI110")
  ti_match = re.search(r'TI\d+', filename, re.IGNORECASE)
  if ti_match:
      return ti_match.group(0).upper(), revision

  # Fallback: filename without revision and extension
  base_name = re.sub(pattern, '', filename, flags=re.IGNORECASE) if match else filename
  return base_name.replace('.pdf', '').strip(), revision
